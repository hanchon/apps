...
export const TransactionInspector = ({ sender, receiver, token, fee, balance, feeBalance, ...props }: ReturnType<typeof useSend>['__DEBUG__']) => {
  const [isOpen, setIsOpen] = React.useState(false);
  return (
    <>
      {!isOpen && <button className="fixed top-0 left-0 bg-slate-600 p-4 text-white font-bold z-50" onClick={() => setIsOpen(!isOpen)}>Transaction inspector</button>}
      {isOpen && (
        <div className="fixed top-0 left-0 z-40 h-full w-72 bg-slate-600 text-white overflow-y-auto p-4  text-xs">
          <button className="absolute top-0 right-0 bg-slate-600 p-4 text-white font-bold z-50" onClick={() => setIsOpen(!isOpen)}>Close</button>
          ...
          ['Fee Balance Query', props.feeBalanceQuery]] as const).map(([title, query]) => (
